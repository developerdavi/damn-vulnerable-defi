...
describe('[Challenge] Unstoppable', function () {
  ...
  it('Exploit', async function () {
    /**
     * These lines (UnstoppableLender.sol: 38-39):
     *
     * // Ensured by the protocol via the `depositTokens` function
     * assert(poolBalance == balanceBefore);
     *
     * Add a vulnerability. It's possible to transfer tokens directly to the contract
     * without calling `depositTokens`. As the contract is relying on the `depositTokens`
     * function to update the `poolBalance` variable, it's possible to stop the contract
     * from offering flash loans and block this check forever.
     */
